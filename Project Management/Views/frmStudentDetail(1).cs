...
namespace Project_Management.Views
{
    ...
    {
        // dùng dictionary để hiển thị list điểm tương ứng với project
        ...
        public frmStudentDetail(Student st)
        {
            ...
            if(st.project.Count!=0)
            {
                foreach (Project item in st.project)
                {
                    try
                    {
                        dicProject.Add(item.name, item.mark);
                ...
                {
                    txtMark.Text = dicProject[dicProject.Keys.ElementAt(0)].ToString();
                }
        ...
        private void cbListProject_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (dicProject.Count > 0)
            {
                ...
                this.txtMark.Text = dicProject[dicProject.Keys.ElementAt(this.cbListProject.SelectedIndex)].ToString();
